...
fn blank_spec(tileset: u8, width: u16, height: u16, terrain_type: u16) -> isom::MapNewSpec {
    isom::MapNewSpec {
        ...
        height,
        terrain_type,
        title: "새 맵 마법사".to_string(),
        description: "generated by eud-agent".to_string(),
        players: vec![
            isom::MapNewPlayer {
                slot: 0,
...
fn map_new_creates_a_verified_blank_map_for_every_tileset() {
    ...
    for tileset in 0_u8..8 {
        ...
        assert_eq!(
            &ownr[..8],
            &[6, 5, 6, 0, 0, 0, 0, 0],
            "human/computer/human then inactive"
        );
        let side = sections.get("SIDE").unwrap();
        assert_eq!(&side[..3], &[5, 0, 2], "userSelectable/zerg/protoss");
...
fn map_new_rejects_invalid_specs_without_writing() {
    ...
    unknown_force.players[0].force = 3;
