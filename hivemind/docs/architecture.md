...
A native adapter publishes its resumable session identity as soon as the CLI reports it, before any output, and the runtime records it as that run's boundary; a run that is cancelled or dies keeps that boundary (receipt state `interrupted`) instead of refusing the conversation, while a session the run did not ask to resume is never adopted. Each chat turn persists the boundary it reached even when the turn fails.
