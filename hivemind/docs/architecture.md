...
- `player_setup` tool (features/09, EUD-089): start-location units (214) + OWNR controllers via `IsomTerrain.exe playeredit` — the SAME rails/journal path as location_write. Exists because eudplib refuses to compile a map with no Human player + start location; the `[build]` prompt section (EUD-088) routes that build failure here.
