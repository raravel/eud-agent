...
`eud-agent` is a standalone Windows Tauri 2 application for native EUD map authoring. It owns project state, epScript files, sparse DAT edits, build generation, review/rollback, map tooling, RAG, and Codex orchestration.
...
- `WorkspaceManager`: durable Codex workspace plus read-only source mirror.
...
4. resolve and authenticate Codex.
