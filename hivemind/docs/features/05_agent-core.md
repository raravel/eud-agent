...
The agent core is in-process Rust. Tauri commands feed `SessionEngineManager`; Codex tool calls route through one `SessionToolRuntime` backed by `ToolServices`.
...
The manifest MainFile is the composition root regardless of filename. Codex should:
...
- Session records persist conversation log, Codex thread, pending requests, project, and kind.
