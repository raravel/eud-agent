...
- **No-project chip**: the editor-up `list` returning `ERROR: no project` is an expected steady state, NOT a failure — the poll routes it to the store's no-project path (`list{error}` → `hasProject=false`) and NEVER logs an "IPC command failed (list)" line. The header connection chip reads "연결됨 · 프로젝트 없음" while `editorConnected && !hasProject` (suffix gated on `editorConnected` so a downed editor stays "연결됨", covered by the ConnectionNotice banner). A genuine (non-no-project) `list` failure is still logged.
...
- **에디터 켜기 (Header)**: a header button invokes `launch_editor`, which spawns `<editor_path>\EUD Editor 3.exe` detached (cwd = install root; never waited on, so the editor outlives the app). Disabled while `editorConnected` (single-instance topology — no re-launch) or a launch is in flight ("여는 중…"); the existing heartbeat poll flips `editorConnected` once the bridge comes up (the "connected" half of done). Stable error codes (`editor path not configured` / `editor executable not found`) map to Korean toasts, never rendered raw.
