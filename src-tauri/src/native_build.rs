...
pub fn generate_native_build_with_python(
    ...
    write_atomic_bytes(
        &build_dir.join(CHK_SECTION_GUARD_PLUGIN),
        CHK_SECTION_GUARD_SOURCE.as_bytes(),
    )
    .map_err(|error| error.to_string())?;
...
/// Generated plugin, loaded first by every build, that keeps the output CHK's section
/// chain finite.
const CHK_SECTION_GUARD_PLUGIN: &str = "ChkSectionGuard.py";

/// eudplib's `CHK.savechk` ends the CHK with a fake `ISOM` section whose length is a
/// random negative number (`randint(0, 0xFFFFFFFF) | 0x80000000`). StarCraft follows
/// it, so when the jump lands inside the CHK instead of before its start (about
/// `len / 2^31` of builds; 0.25% for a 5 MiB CHK), the game walks the payload's zero
/// headers in a cycle and hangs while listing the map's folder. The guard draws the
/// fake length again until the jump leaves the CHK. It runs before freeze, which
/// keys its protection to the CHK it is handed, so nothing is patched afterwards.
const CHK_SECTION_GUARD_SOURCE: &str = r#"# Generated by eud-agent: keeps the CHK section chain from looping in StarCraft.
import warnings

try:
    from eudplib.core.mapdata.chktok import CHK as _Chk
    _save_chk = _Chk.savechk
except (ImportError, AttributeError):
    warnings.warn("eud-agent: eudplib CHK.savechk not found; the CHK section guard is off")
else:
    def _save_chk_with_leaving_tail(self):
        for _ in range(64):
            chk = _save_chk(self)
            if chk[-8:-4] != b"ISOM":
                return chk
            if len(chk) + int.from_bytes(chk[-4:], "little", signed=True) < 0:
                return chk
        raise RuntimeError("eud-agent: the CHK's trailing ISOM section kept pointing inside the CHK")

    _Chk.savechk = _save_chk_with_leaving_tail
"#;
...
mod tests {
    ...
    fn dependency_bootstrap_precedes_plugins_and_ordered_python_entrypoints() {
        ...
        .unwrap();
        let chk_guard = eds.find("[ChkSectionGuard.py]").unwrap();
