...
impl TaskStateCompilerInput<'_> {
    pub fn prompt(&self) -> Result<String, String> {
        ...
        Ok(format!(
            "You are a bounded active-task state compiler. Return exactly one JSON object matching the supplied schema. Do not call tools, inspect files, infer authority, or emit Markdown/prose. Use only the inline input. Preserve exact target-set membership and counts. Emit only upsert, supersede, remove, or close_request operations. Every upsert must include valid exact provenance from the supplied user turn, approved plan, or listed project artifact; never claim accepted or promoted state without qualifying provenance. Active state grants no write, map, attachment, editor, journal, or tool authority.\n\n[compiler input]\n{input}\n\n[compiler semantics]\nThis is a one-pass extraction of session context, not a new planning or design task. Emit each changed requirement once; do not duplicate a fact across collections. Preserve every explicit target-set member. Copy exact quotes verbatim; do not paraphrase provenance or invent hashes. Use existing IDs when updating facts. Set baseRevision to previousProjection.revision. Explicit user requirements are active; unapproved alternatives are proposed. A completed foreground answer is not user acceptance. Accepted status requires approved-plan or accepted-journal provenance. close_request only supersedes existing active/proposed blockers that carry this request's approved-plan or accepted-journal provenance; it does not complete goals. Omit close_request when no such blocker exists. If nothing changed, return the unchanged baseRevision with an empty operations array. Return the delta directly without reconsidering equivalent IDs, quote lengths, or classifications."
        ))
    }
}
